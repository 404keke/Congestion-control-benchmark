...
int main(int argc, char *argv[]){
    ...
    // I set trace1 work form 0~300s; flow2 work from 0~200s; flow3 work from 50~300s. You can change it by youself

    DqcTrace trace1;
    std::string log=prefix+std::to_string(test_pair);
    // If u want add morre output data, just add E_DQC_RTT, E_DQC_GOOD...
    ...
    test_pair++;
    InstallDqc(cc,nodes.Get(0),nodes.Get(1),sendPort+1,recvPort+1,appStart,200,&trace2,max_bps,sender_id,emucons);
    sender_id++;
    ...
    test_pair++;
    InstallDqc(cc,nodes.Get(0),nodes.Get(1),sendPort+2,recvPort+2,appStart+50,appStop,&trace3,max_bps,sender_id,emucons);
    sender_id++;
